...
class ParetoPartialBeamSynthesizerV2(Synthesizer):
    """
    ...
    def _diversity_truncate(
        ...
        """
        Preserve extreme points + select remaining by crowding distance
        """
        ...
        if len(selected) >= self.beam_width:
            return list(selected)[: self.beam_width]
        
        # Apply crowding distance to remaining
        distances = self._crowding_distance(remaining)
        ranked = sorted(
            zip(remaining, distances),
            key=lambda x: x[1],
            reverse=True,
        )
